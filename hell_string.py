...
def main():
    ...
    print("Hello!! Welcome to XD Session Generator\n")
...
def generate_hellbot_session():
    print("!!! XDBOT SESSION !!!")
    print("One session for all XDBot's Project.")
    api_id = int(input("\nEnter APP ID here: "))
    api_hash = input("\nEnter API_HASH here: ")
    with PClient(name="XDuser", api_id=api_id, api_hash=api_hash, in_memory=True) as hell:
        print("\nYour XDBOT SESSION is saved in your telegram saved messages.")
...
def generate_pyro_session():
    ...
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with PClient(name="XDuser", api_id=APP_ID, api_hash=API_HASH, in_memory=True) as hellbot:
        print("\nYour XDBot Session Is sent in your Telegram Saved Messages.")
        XDbot.send_message(
            "me",
            f"#XDBOT #PYROGRAM\n\n`{XDbot.export_session_string()}`",
        )


def generate_telethon_session():
    print("\nTelethon Session For XDBot!")
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
...
def generate_insta_session():
    print("Instagram Session For XDBot!")
    cl = IClient()
...
def hellbot_session(session):
    ...
    error_msg = "Error in generating session! Report it in XD Chats"
    ...
    if len(session) in pyro_format.keys():
        ...
        return f"=={XDbot('XD')}{new_session}{XDbot('bot')}=="
    else:
        return error_msg
